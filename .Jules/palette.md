...
## 2026-09-02 - Screen Reader Optimization for Repetitive Text in Links
**Learning:** In card-based layouts wrapped in `<Link>` components (like `Topics.tsx`), screen readers announce all text content inside the link consecutively. This can lead to screen reader fatigue when generic Call-to-Action text (e.g., "Przejdź do przedmiotu →") and decorative emojis are read repeatedly for every card.
**Action:** Always apply `aria-hidden="true"` to purely decorative icons and generic repetitive CTA text inside card-based links, allowing users to focus on the meaningful content (title and description).

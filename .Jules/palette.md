...
## 2026-08-11 - Screen Reader Noise in Link Cards
**Learning:** In card-based layouts wrapped in semantic `<Link>` components (like `Topics.tsx`), screen readers repeatedly announce purely decorative icons and generic CTA texts (e.g., "Przejdź do przedmiotu →") alongside the meaningful title and description. This causes high cognitive load and excessive screen reader noise.
**Action:** When wrapping entire cards in links, explicitly apply `aria-hidden="true"` to purely decorative visual elements (like emojis/icons) and redundant/generic CTA text inside the card, allowing the screen reader to focus only on the meaningful title and description content of the link.

...
## 2026-09-11 - Screen Reader Flow for Link-Wrapped Cards
**Learning:** When using large card-based layouts wrapped entirely in a `<Link>` element, screen readers read all textual content within the card sequentially. Including repetitive call-to-action (CTA) text (e.g., 'Learn more', 'Start learning') and decorative icon/image wrapper elements creates unnecessary audio noise and disrupts the reading flow of the core content.
**Action:** Apply `aria-hidden="true"` to purely decorative icons and generic repetitive CTA text inside link-wrapped cards, allowing screen reader users to focus exclusively on meaningful content.

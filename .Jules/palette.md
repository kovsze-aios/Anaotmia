...
## 2026-09-19 - Decorative Elements in Interactive Controls
**Learning:** Found interactive elements (like the topbar hamburger menu and sidebar close buttons) containing decorative elements like raw text ('✕', '☰ ') and structural spans that lacked `aria-hidden='true'`. This adds visual noise to screen reader announcements and hurts accessibility.
**Action:** When adding decorative text, icons, or structural spans inside of interactive components (such as `<button>` or `<Link>`) that already use an `aria-label` or visible textual label, ensure the decorative elements have `aria-hidden='true'`.

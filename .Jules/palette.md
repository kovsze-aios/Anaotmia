...
## 2026-09-03 - Screen Reader Noise Reduction in Card Links
**Learning:** Found repetitive CTA text ("Przejdź do przedmiotu →") and purely decorative icons within `<Link>` wrapper components on the Topics page. When a screen reader focuses on the card link, it reads out all the visible text content sequentially, resulting in repetitive noise that dilutes the primary information (title and description) and degrades the navigational experience.
**Action:** Apply `aria-hidden="true"` to purely decorative visual elements (like large emojis/icons) and generic repetitive CTA text inside complex link cards to allow users to focus on the meaningful content (title, description).

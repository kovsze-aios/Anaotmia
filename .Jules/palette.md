...
## 2026-08-20 - Repetitive CTAs and Decorative Icons in Card Links
**Learning:** In card-based layouts wrapped in `<Link>` components (e.g., in `Topics.tsx`), screen readers announce all content sequentially. Generic repetitive CTA text (like "Przejdź do przedmiotu →") and purely decorative icons add noise and degrade the navigation experience for keyboard/screen reader users.
**Action:** When wrapping entire cards in links or buttons, apply `aria-hidden="true"` to purely decorative icons and generic repetitive CTA text, allowing users to focus on meaningful content (title and description) that is inherently accessible through the link.

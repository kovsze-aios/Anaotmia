...
## 2024-08-30 - Screen Reader Noise in Card Grids
**Learning:** Discovered that in card-based grid layouts wrapped entirely in Link components (like in Topics.tsx), screen readers announce all decorative text elements (like icons and repetitive CTA text) alongside the actual meaningful title and description, creating significant auditory clutter.
**Action:** Always apply aria-hidden="true" to purely decorative elements (icons) and generic repetitive CTA text within interactive link/button wrappers so screen readers only focus on the meaningful content.

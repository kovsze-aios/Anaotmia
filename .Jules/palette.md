...
**Action:** Standardize on `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-400 dark:focus-visible:ring-zinc-600` across all custom interactive components to ensure consistent keyboard accessibility.
## 2026-08-22 - Screen Reader Redundancy in Card Grids
**Learning:** Found multiple `<Link>` components functioning as cards (`subject-card`, `topicbox`) in `Topics.tsx` that contained redundant "call to action" text inside them. Since the card's main content is already screen reader accessible, adding generic text like "Przejdź do przedmiotu" inside the link creates an excessively verbose and poor screen reader experience.
**Action:** Always apply `aria-hidden="true"` to generic, repetitive CTA text ("Read more", "Go to section") when it exists inside a larger `<Link>` block that already has descriptive text (like a title and summary). Also, explicitly hide decorative icon wrappers.

...
**Action:** Standardize on `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-400 dark:focus-visible:ring-zinc-600` across all custom interactive components to ensure consistent keyboard accessibility.
## 2026-07-20 - Global Navigation Screen Reader Experience
**Learning:** Found that generic repetitive text within structural components mapped using `Link` components (`subject-card__cta`, `topicbox__cta`), alongside purely decorative icons can pollute the audio tree for screen readers. Using these creates significant audio clutter for visual learners.
**Action:** When creating structured cards with semantic wrappers (`<Link>`), always use `aria-hidden="true"` to mute purely decorative icons and generic repetitive CTA text (e.g. "Przejdź do przedmiotu →"), to allow users to focus on meaningful text content.

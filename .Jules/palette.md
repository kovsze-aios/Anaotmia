...
## 2026-09-06 - Extraneous visual link semantics
**Learning:** Found that visual/decorative icons and generic CTA text (like 'Przejdź do przedmiotu') within navigational subject cards create excessive noise for screen reader users, who already receive context from the card's subtitle and title.
**Action:** Apply `aria-hidden="true"` to purely decorative icons and generic repetitive CTA text inside card-based layouts wrapped in `<Link>` components to allow users to focus on meaningful content.

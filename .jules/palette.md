...
## 2026-09-13 - Generic/Repetitive Link CTA Screen Reader Experience
**Learning:** Found that card-based layout components wrapped in `<Link>` often have generic repetitive CTA text like "Czytaj dalej" (Read more) and purely decorative imagery alongside meaningful content. When a screen reader focuses on the card, these extra elements cause unnecessary verbosity and cognitive overload.
**Action:** Always apply `aria-hidden="true"` to purely decorative icons and generic repetitive CTA text inside card-based link elements, allowing users to focus on meaningful content like the title and description.

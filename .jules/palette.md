...
## 2026-09-22 - Screen Reader Noise from Decorative Text and Icons
**Learning:** Decorative icons and structural characters (like '✕' or '☰') inside interactive elements, as well as repetitive CTA texts in card layouts, add unnecessary noise for screen reader users when the interactive element's purpose is already clear from other content or an `aria-label`.
**Action:** Always apply `aria-hidden="true"` to purely decorative icons, structural characters, and generic repetitive CTA text inside interactive elements to improve the screen reader experience.

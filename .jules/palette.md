...
**Action:** Standardize on `focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-zinc-400 dark:focus-visible:ring-zinc-600` across all custom interactive components to ensure consistent keyboard accessibility.
## 2026-09-20 - Repetitive Content in Actionable Cards
**Learning:** Found multiple instances of card-based navigation components (`Topics.tsx`, `TheoryIndex.tsx`) where an entire card acts as a `<Link>`, but inner decorative icons and generic repetitive CTA texts (e.g. "Go to subject") were readable by screen readers. This creates a noisy, unnecessarily verbose experience.
**Action:** When an entire block/card is clickable and has meaningful text content (title, description), apply `aria-hidden="true"` to any purely visual decorative spans/icons and repetitive generic CTA text within the card to streamline screen reader output.

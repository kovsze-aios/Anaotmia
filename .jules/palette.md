...
## 2026-09-15 - Screen Reader Noise in Complex Cards
**Learning:** Found that `<Link>` wrappers containing card-based content (like the `subject-card` and `topicbox` components in `Topics.tsx`) often include repetitive visual elements (e.g., generic decorative icons, "Learn more" CTA text). When a screen reader reads the link, it announces all the child text, creating unnecessary noise.
**Action:** Always add `aria-hidden="true"` to purely decorative visual elements (like generic icons) and generic repetitive call-to-action text (like "Check it out") inside large clickable cards. This ensures screen readers only read the meaningful titles and descriptions.

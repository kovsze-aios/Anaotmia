...
## 2026-09-24 - Screen Reader and Decorative Characters
**Learning:** Found multiple instances of decorative characters (like `✕`, `☰`, `←`) inside interactive elements (`button`, `a`, `Link`) that were not explicitly hidden from screen readers. This causes screen readers to read these characters alongside the element's `aria-label` or visible text, confusing users.
**Action:** Always wrap purely decorative text characters and structural icons inside interactive elements with a `<span aria-hidden="true">` to ensure they are ignored by screen readers.

...
## 2025-02-12 - Derived array operations in React
**Learning:** Performing array operations like `.filter()` or `.map()` directly inside a React component's render body forces React to re-allocate memory and re-calculate the array on every single render. When passed as a prop, this breaks referential equality and causes unnecessary re-renders in child components.

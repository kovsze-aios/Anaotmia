...
## 2025-02-12 - Unmemoized Derived Arrays in Render
**Learning:** Computing derived arrays (like using `.filter()` or `.map()`) directly in the render body creates a new array reference on every render. If this array is passed down as a prop, it can cause unnecessary re-renders of child components that depend on referential equality.

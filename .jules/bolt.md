...
## 2025-02-13 - Optimized Excerpt Generation
**Learning:** Using global regex replace (`text.replace(/\s+/g, " ").trim()`) on large text strings creates huge intermediate string allocations that trigger GC pauses and block the main thread.
**Action:** When extracting a short excerpt from potentially massive text content, avoid applying full-string regexes. Instead, use a bounded zero-allocation `for` loop with `charCodeAt` to build the excerpt character by character up to the maximum required length (O(max)).

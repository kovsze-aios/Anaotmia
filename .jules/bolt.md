...
## 2025-02-12 - Word Count Optimization on Massive Strings (Avoid Regex when Creating Excerpts)
**Learning:** Using global regex replacements (e.g. `text.replace(/\s+/g, " ")`) to collapse whitespace and create short excerpts from massive domain texts causes severe main thread blockage and memory overhead (O(N)), especially when processing thousands of entries during index creation.
**Action:** Replace `replace(/\s+/g, " ")` with an optimized bounded `charCodeAt` loop that only scans up to the `max` length of the desired excerpt. This reduces complexity to O(max) and prevents full-text scanning and allocations.

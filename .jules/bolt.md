...
## 2024-05-30 - [Optimize massive string regex by slicing first]
**Learning:** Running global regex replacements (like `.replace(/\s+/g, " ")`) over massive text strings (e.g., entire `academic_detail` textbook chapters) just to extract a short 160-character excerpt causes severe thread blocking and CPU/memory overhead during index initialization.
**Action:** When extracting short excerpts from massive strings, apply `.trimStart()` and `.slice(0, max * 3)` *before* applying any complex regex sanitization to keep the operation O(1) instead of O(N).

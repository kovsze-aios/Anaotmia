...
## 2025-02-12 - Prevent Re-renders with useMemo
**Learning:** Performing array operations like `.filter()` or `.map()` directly in the render body of a React component can cause unnecessary re-renders in downstream dependencies because it creates a new array reference on every single render.
**Action:** Always wrap derived array computations in `useMemo` when calculating things like `filtered` arrays to preserve referential equality and avoid O(N) operations unless the dependencies genuinely change.

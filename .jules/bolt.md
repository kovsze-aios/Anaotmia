...
## 2025-02-12 - Regex Lookbehind Performance
**Learning:** In modern JavaScript engines like V8, replacing negative lookbehinds `(?<!
)` with capture groups `([^
])` does not guarantee a performance boost and might even add slight overhead due to capturing allocations. However, it resolves known compatibility issues with older browsers (e.g., Safari < 16.4) that do not support regex lookbehinds.
**Action:** Prioritize capture groups over lookbehinds for broad browser compatibility, but do not strictly label it a micro-optimization for performance without concrete V8 profiling.

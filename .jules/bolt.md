...
## 2026-09-23 - Array Filter Memoization
**Learning:** Performing array operations like `.filter()` on potentially large arrays (e.g., loaded questions) inside a React component's render body without memoization causes new array references to be created on every render, triggering unnecessary re-renders of downstream components.
**Action:** Always wrap derived array computations involving `.filter()` or `.map()` in `useMemo` to preserve referential equality when dependencies haven't changed, especially for domain-heavy lists like CKE questions.

...
## 2025-02-12 - Derived Array Filtering Overhead in React
**Learning:** Performing array operations like `.filter()` or `.map()` directly in the render body of a React component (like `MaturaSubjectView`) on every render creates a new array reference each time. This breaks referential equality and forces unnecessary downstream re-renders, causing performance drops, especially when unrelated state changes trigger a re-render.
**Action:** When filtering or mapping data derived from props or state that doesn't change on every render, always wrap the computation in a `useMemo` block with correct dependencies.

...
## 2025-02-12 - Regex vs substring on massive OCR text strings
**Learning:** Using global regex replace on a massive multi-megabyte string just to extract a preview (e.g. `text.replace(/\s+/g, ' ').slice(0, 160)`) performs significantly worse (3s vs 0.8ms) than slicing first and executing the regex on the small subset.
**Action:** Always constrain massive strings with `.slice` before executing expensive global regular expressions, even if it introduces edge cases regarding leading strings.

...
## 2025-02-12 - Word Count Calculation on Massive Strings
...
**Action:** Always prefer regex matching like `(text.match(/\S+/g) || []).length` over `split` and `filter` when counting words or iterating over simple string patterns in massive texts to reduce memory overhead.

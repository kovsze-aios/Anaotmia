...
## 2025-02-23 - Massive String Processing Performance
**Learning:** To avoid severe CPU and memory overhead during data parsing or search index initialization, always slice massive strings to a safe maximum length (e.g., `max * 3`) before applying global regex replacements or complex string manipulations.
**Action:** Always slice strings to a safe maximum length *before* applying regex for previews/excerpts.

...
## 2025-02-13 - Array Filtering Re-renders
**Learning:** `Array.filter()` inside a component render body creates a new array instance on every single render. When passed down as props or used to map over UI elements, this causes unnecessary main thread computation and forces React to reconcile the entire list even if the source data hasn't changed.
**Action:** Always wrap derived array computations like `filter` or `map` in `useMemo` when they rely on state or props, preserving referential equality and avoiding performance degradation on rapid re-renders (like scrolling or expanding accordions).

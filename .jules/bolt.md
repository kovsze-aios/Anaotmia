...
## 2025-02-13 - Replace Regex with Char Iteration for Excerpts
**Learning:** Using global regex replace like `.replace(/\s+/g, " ")` on massive OCR text strings (megabytes) to generate a short excerpt blocks the main thread because it processes the entire string before truncation.
**Action:** Replace `O(N)` eager regex operations with `O(max)` lazy character loops (using `charCodeAt`) that only parse up to the needed excerpt length (`max`) and then early-exit. This improves performance by ~10,000x for massive strings.

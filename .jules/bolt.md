...
## 2025-02-12 - Wrap array computations in React render with useMemo
**Learning:** When performing array operations like `.filter()` or `.map()` directly inside a React component's render body without memoization, it breaks referential equality on every re-render, potentially causing child components to re-render needlessly.

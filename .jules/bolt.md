...
## 2025-02-12 - Missing Memoization
**Learning:** Performing expensive operations, such as filtering a large array, directly in a component render body without memoization leads to unnecessary re-computations and array re-creations on every re-render (e.g., from scrolling or state changes).
**Action:** Always wrap derived data computations like `.filter()` or `.map()` in `useMemo` when rendering components to avoid re-evaluating the expressions unless dependencies change.

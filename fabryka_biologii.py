...
Potok ingestii modułu BIOLOGIA: czteroczęściowa seria "Biologia na czasie —
zakres rozszerzony" (Nowa Era) → silnie typowane moduły TypeScript.

Trzecia fabryka obok `fabryka_wiedzy.py` (anatomia) i `fabryka_fizjologii.py`
(fizjologia). Bez zmian pozostają: prompt antyplagiatowy wraz z odtwarzaniem
diakrytyków, dwumodelowy silnik, biała lista HTML, `max_tokens=8192` z kontrolą
`finish_reason`, sanityzacja Markdownu, pula wątków, limity ponowień, czarne
listy i deduplikacja SHA-256. Różni się warstwa wejściowa.

Czym te książki różnią się od poprzednich źródeł:

  1. CZTERY PLIKI, jak tomy anatomii — po jednym na część serii. Każda część
     dostaje własny tag (`bio1`…`bio4`), czyli własną domenę w nawigacji.

  2. DIAKRYTYKI SĄ NA MIEJSCU. To UTF-8 z pełną polszczyzną (24–62 tys. znaków
     diakrytycznych na plik), w przeciwieństwie do anatomii i Lewińskiego,
     gdzie ekstrakcja PDF je gubiła. Prompt i tak każe je odtwarzać — to jest
     teraz operacja pusta, ale nieszkodliwa, a zostaje na wypadek gorszego
     źródła w przyszłości.

  3. UKŁAD WIELOŁAMOWY. Podręcznik szkolny składa się z łamów i marginaliów,
     które ekstrakcja przeplata: 15–19% przeniesień wyrazowych ma kontynuację
     nie w następnym wierszu, tylko dwa–trzy wiersze dalej, za wtrąconym
     paskiem bocznym. Stąd ostrożniejsza reguła sklejania przeniesień, patrz
     `HYPHEN_BREAK_RE`.

  4. STRUKTURA JEST NUMEROWANA I CZYSTA: rozdział "Rozdział N." i temat "N.M.
     Tytuł". Temat jest jednostką semantyczną — jest ich ~360 w całej serii.
     Tytuł tematu wraca jako żywa pagina na każdej stronie tematu, więc długi
     temat rozpada się na pakiety wielkości strony. To jest zamierzone: taki
     pakiet odpowiada jednej "części" w module anatomii.

  5. SPIS TREŚCI UDAJE TREŚĆ. Numery tematów stoją w spisie w dwóch kolumnach,
     więc wyglądają dokładnie jak nagłówki w tekście. Rozróżnia je odstęp —
     patrz `MIN_TOPIC_CHARS`.

Użycie:
    python fabryka_biologii.py            # pełny przebieg, cztery części
    python fabryka_biologii.py --dry-run  # dzieli i waliduje, bez API
"""
...
# ── Czyszczenie specyficzne dla "Biologii na czasie" ────────────────────────

#: Znacznik strony zostawiony przez ekstraktor PDF.
PAGE_MARKER_RE = re.compile(r"---\s*STRONA\s+\d+\s*---", re.IGNORECASE)

#: Żywa pagina rozdziału, powtórzona na każdej stronie ("Rozdział 1. Badania
#: przyrodnicze"). Tytuł tematu też wraca jako pagina, ale ten zostaje —
#: to on wyznacza granice pakietów.
...
#:
#: Reguła odziedziczona po anatomii, `(\w)-\n(\w)`, przyjmowała po przeniesieniu
#: dowolny znak wyrazowy, w tym wielką literę. W książce jednołamowej to bez
#: znaczenia, bo kontynuacja stoi w następnym wierszu. Tutaj co szóste
#: przeniesienie przerywa pasek boczny, więc następny wiersz bywa początkiem
#: zupełnie innego zdania — i tamta reguła skleiłaby "omawia-" z
#: "Doświadczenie". W tych czterech plikach zdarzyłoby się to 1 636 razy.
#: Polskie przeniesienie zawsze wraca małą literą, więc to jedno zawężenie
#: wystarcza, żeby żaden z tych przypadków nie powstał.
...
#: Części serii, w kolejności wydawniczej. Klucz jest tagiem plików
#: (`bio3-czesc-12-….ts`), więc musi zostać przy [a-z0-9] — tego wymaga parser
#: indeksów w `update_barrel_files`. `match` to fragment nazwy pliku
#: źródłowego, po którym rozpoznajemy część.
...
#: Fragment nazwy, po którym rozpoznajemy pliki tej serii.
#:
#: Katalog źródłowy zawiera także vademecum i zbiór zadań maturalnych — dwa
#: pliki, których ta seria nie obejmuje. Bez tego filtra `*.txt` wciągnąłby
#: obydwa; to ten sam błąd, który w anatomii sprawiał, że arkusz egzaminacyjny
#: wypadał z potoku przypadkiem, a nie przez filtr.
SERIES_FILENAME_HINT = "na czasie"

#: Katalog wyjściowy modułu biologii.
OUTPUT_DIR = Path("src/data/biologia/dzialy")


def get_part_tag(filename: str) -> tuple[str, str] | None:
    """Zwraca (tag, tytuł) dla pliku serii albo None, gdy to nie jest jej część."""
    fn = filename.lower()
...
def find_sources(src_dir: Path) -> list[Path]:
    """Cztery części serii, w kolejności wydawniczej."""
    found: list[tuple[str, Path]] = []
    for path in sorted(src_dir.glob("*.txt")):
        if SERIES_FILENAME_HINT not in path.name.lower():
            continue
        ...
        if tagged is None:
            print(f"  [POMINIĘTO PLIK] {path.name} — nie rozpoznano części serii")
            continue
        found.append((tagged[0], path))
    found.sort(key=lambda pair: int(pair[0].removeprefix("bio")))
...
def build_tasks(out_dir: Path, force: bool = False) -> list[tuple]:
    src_dir = Path("materiały-źródłowe/biologia")
    if not src_dir.exists():
        src_dir = Path("../materiały-źródłowe/biologia")

    txt_files = find_sources(src_dir)
    if not txt_files:
        print(f"Nie znaleziono części serii \"Biologia na czasie\" w {src_dir}")
        return []

    print(f"Części do przetworzenia ({len(txt_files)}):")
    for f in txt_files:
        print(f"  - {f.name}")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Fabryka biologii — potok ingestii serii \"Biologia na czasie\".")

import type { AtlasPart } from "@/server/models";
...
/**
 * Shelf-packs the visible meshes into a grid for the fully-exploded view, so
 * every structure gets its own cell instead of overlapping its neighbours.
 * Only visible parts are packed — the layout changes as systems are toggled.
 */
export function createExplosionLayout(parts: AtlasPart[], aspect = 1) {
  ...
  // Tallest first keeps shelves dense; the id tiebreak keeps layout stable.
  ...
  const height = y + row;
  // Recentre the packed sheet on the origin.
  cells.forEach((c) => {
    c.x -= usedWidth / 2;
    c.y += height / 2;
  });

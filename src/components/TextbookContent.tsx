...
function getSectionWordCount(section: TextbookSection): number {
  ...
  // Bolt: Use a zero-allocation `for` loop iterating over character codes instead of regex match
  // to count words in large OCR text. This avoids excessive memory allocation (arrays of all words)
  ...
  const countWords = (text: string) => {
    ...
    for (let i = 0; i < text.length; i++) {
      ...
      const isWhitespace = (code >= 9 && code <= 13) || code === 32 || code === 160;
      if (isWhitespace) {
        inWord = false;
      } else {
        if (!inWord) {

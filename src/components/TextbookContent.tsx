...
function getSectionWordCount(section: TextbookSection): number {
  ...
  // Bolt: Zero-allocation word count for massive text strings.
  // We use a manual character code loop because regex match() still allocates a massive array
  // of all matches before we can read .length, causing GC spikes on large OCR chunks.
  const countWords = (text: string) => {
    let wordCount = 0;
    let inWord = false;
    for (let i = 0; i < text.length; i++) {
      const code = text.charCodeAt(i);
      // ASCII whitespace: Space (32), Tab (9), LF (10), CR (13), etc. Also NBSP (160)
      if (code <= 32 || code === 160) {
        inWord = false;
      } else {
        if (!inWord) {

...
const SUBJECT = "biologia" as const;
const SUBJECT_LABEL = "Biologia";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const url = absoluteUrl(`/theory/biologia/${section.id}`);
...
export default async function BiologiaChapterPage({ params }: Props) {
  const { id } = await params;
